...
import { getMovieDetails, getTVShowDetails, getTVSeasonDetails, IMAGE_BASE_URL_ORIGINAL } from '@/lib/tmdb';
...
export default async function PlayerPage({ params, searchParams }: PlayerPageProps) {
  ...
  if (type === 'movie') {
    ...
    embedSrc.p1 = `https://vidsrc-embed.ru/embed/movie/${tmdbId}`;
    ...
    embedSrc.p1 = `https://vidsrc-embed.ru/embed/tv/${tmdbId}/${seasonNumber}/${episodeNumber}`;
  ...
  const serverButtonClasses = "w-full text-center px-4 py-2.5 rounded-lg font-semibold transition-all duration-300 ease-in-out transform hover:scale-105 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg data-[state=active]:shadow-primary/40 data-[state=inactive]:bg-card/70 data-[state=inactive]:hover:bg-primary/20 data-[state=inactive]:text-muted-foreground data-[state=inactive]:hover:text-foreground data-[state=inactive]:border data-[state=inactive]:border-border/50";
  const backdropUrl = backdropPath ? `${IMAGE_BASE_URL_ORIGINAL}${backdropPath}` : null;
    ...
    <>
      ...
      <PageContainer>
        ...
        <Tabs defaultValue={selectedPlayer} className="w-full">
          <TabsList className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-8 gap-2 sm:gap-4 max-w-4xl mx-auto mb-6 bg-transparent p-0 h-auto">
            ...
            <TabsTrigger value="2" asChild>
                <Link href={`?player=2&s=${seasonNumber}&e=${episodeNumber}`} scroll={false} className={serverButtonClasses}>
                  Server 2
                </Link>
            </TabsTrigger>
            <TabsTrigger value="3" asChild>
                <Link href={`?player=3&s=${seasonNumber}&e=${episodeNumber}`} scroll={false} className={serverButtonClasses}>
                  Server 3
                </Link>
            </TabsTrigger>
            <TabsTrigger value="4" asChild>
                <Link href={`?player=4&s=${seasonNumber}&e=${episodeNumber}`} scroll={false} className={serverButtonClasses}>
                  Server 4
                </Link>
            </TabsTrigger>
            <TabsTrigger value="5" asChild>
                <Link href={`?player=5&s=${seasonNumber}&e=${episodeNumber}`} scroll={false} className={serverButtonClasses}>
                  Server 5
                </Link>
            </TabsTrigger>
            <TabsTrigger value="6" asChild>
                <Link href={`?player=6&s=${seasonNumber}&e=${episodeNumber}`} scroll={false} className={serverButtonClasses}>
                  Server 6
                </Link>
            </TabsTrigger>
            <TabsTrigger value="7" asChild>
                <Link href={`?player=7&s=${seasonNumber}&e=${episodeNumber}`} scroll={false} className={serverButtonClasses}>
                  Server 7
                </Link>
            </TabsTrigger>
            <TabsTrigger value="8" asChild>
                <Link href={`?player=8&s=${seasonNumber}&e=${episodeNumber}`} scroll={false} className={serverButtonClasses}>
                  Server 8
                </Link>
            </TabsTrigger>
          </TabsList>
          ...
          <div className="relative w-full overflow-hidden rounded-lg bg-black" style={{ paddingBottom: '56.25%' }}>
              ...
              <TabsContent value="7" className="absolute inset-0 w-full h-full m-0">
