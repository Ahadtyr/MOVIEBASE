      ...
      {isFocused && suggestions.length > 0 && (
        <ul className="absolute z-10 w-full mt-1 bg-card border border-border rounded-lg shadow-xl overflow-hidden animate-fade-in max-h-96 overflow-y-auto">
          {suggestions.map(item => {
            const isMovie = 'vote_average' in item; // Distinguish Movie from TVShow
            const href = isMovie ? `/movie/${item.id}` : `/tv-show/${item.id}`;
            // Use a generic placeholder for movies since placeholder movies have no poster_path
            const posterUrl = isMovie ? 'https://placehold.co/40x60.png' : (item as TVShow).posterUrl;
